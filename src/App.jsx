...
import { 
  ...
  Clock, Camera, Image as ImageIcon, AlertTriangle, LogOut, Key, UserPlus, Settings, Users, ShieldCheck
} from 'lucide-react';
...
export default function App() {
  ...
  const handleIdentify = async (e) => {
    ...
    try {
      ...
      // SONDERFALL: Admin Suuger automatisch anlegen, falls noch nicht existent
      if (!memberSnap.exists() && fn.toLowerCase() === 'admin' && ln.toLowerCase() === 'suuger') {
  ...
  const handleSetupPassword = async (e) => {
    ...
    const fn = authForm.firstName.trim();
    const ln = authForm.lastName.trim();
    const email = generatePseudoEmail(fn, ln);
    const memberId = `${fn.toLowerCase()}_${ln.toLowerCase()}`;
      ...
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-6">
        <div className="w-full max-w-sm">
          <div className="text-center mb-10">
            ...
            <p className="text-gray-600 text-[10px] font-bold uppercase tracking-[0.3em] mt-2 italic">Ämme Storage Login</p>
          </div>
        ...
        {activeTab === 'admin' ? (
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {members.map(m => (
                <div key={m.id} className="bg-[#111] border border-gray-800 p-5 rounded-3xl flex justify-between items-center group">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className={`text-[8px] uppercase font-black px-2 py-0.5 rounded-full ${m.hasPassword ? 'bg-green-950 text-green-500' : 'bg-gray-800 text-gray-500'}`}>
                        {m.hasPassword ? 'Registriert' : 'Wartet'}
                      </span>
                      {m.defaultRole === 'admin' && <span className="text-[8px] uppercase font-black px-2 py-0.5 rounded-full bg-orange-950 text-orange-500">Admin</span>}
                    </div>
                  </div>
                  {m.id !== 'admin_suuger' && (
          ...
          <>
            <div className="flex flex-col gap-4 mb-8">
              <div className="relative group">
                ...
                <input type="text" placeholder="Suche..." className="w-full bg-[#161616] p-4 pl-12 rounded-2xl outline-none border border-gray-800 text-white shadow-inner focus:border-orange-500/50" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
              </div>
                  ...
                  <div className="h-48 bg-black relative flex items-center justify-center border-b border-gray-800/50 overflow-hidden">
                    ...
                    {item.status === 'Ausgeliehen' && (
                      <div className="absolute inset-0 bg-orange-950/40 backdrop-blur-[2px] flex flex-col items-center justify-center pointer-events-none">
                        ...
                        <p className="text-[8px] text-white/60 font-bold uppercase tracking-widest">{item.lastActionBy}</p>
                      </div>
                    )}
                  </div>
